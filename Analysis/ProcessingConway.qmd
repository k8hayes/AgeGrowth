...
new_process = new %>%
  ...
  summarise(count_below_dbh = sum(count_below_dbh),
            dbh = mean(tree_1_dbh_cm),
            basal = mean(tree_1_basal_cm)) %>%
  ungroup() %>%
  mutate(species = ifelse(species == "betneo", "bene",
                      ifelse(species == "picmar", "pima", "potr")),
         year = 2024)
```
